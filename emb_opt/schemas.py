...
# %% auto 0
__all__ = ['Embedding', 'DataSourceFunction', 'FilterFunction', 'ScoreFunction', 'PruneFunction', 'UpdateFunction', 'Item',
           'Query', 'Batch', 'DataSourceResponse', 'FilterResponse', 'ScoreResponse', 'PruneResponse',
           'ContinuousUpdateResponse', 'UpdateResponse']
...
class Query(BaseModel):
    ...
    def from_item(cls, item: Item):
        inputs = {
            ...
            'data' : {'_internal' : item.data['_internal']}
        }
        return cls(**inputs)
    
    @classmethod
    def from_parent(cls, embedding: Embedding, parent_query):
        query = cls(embedding=embedding)
        parent_id = parent_query.data['_internal']['id']
        parent_collection = parent_query.data['_internal'].get('collection_index', None)
        query.data['_internal']['parent'] = parent_id
        query.data['_internal']['collection_index'] = parent_collection
        return query
    ...
    def add_collection_index(self, collection_index):
        self.data['_internal']['collection_index'] = collection_index

# %% ../nbs/01_schemas.ipynb 9
...
# %% ../nbs/01_schemas.ipynb 21
PruneFunction = Callable[List[Query], List[PruneResponse]]

# %% ../nbs/01_schemas.ipynb 23
class ContinuousUpdateResponse(BaseModel):
    embedding: Embedding
    parent_query: Query
        
class UpdateResponse(BaseModel):
    results: Union[Batch, List[Item], List[ContinuousUpdateResponse]]
        
    @model_validator(mode='before')
    @classmethod
    def check_type(cls, inputs):
        results = inputs.get('results')
        
        if isinstance(results, Batch):
            pass
        elif isinstance(results, list):
            if isinstance(results[0], Item):
                _ = [Item.model_validate(i) for i in results]
            elif isinstance(results[0], ContinuousUpdateResponse):
                _ = [ContinuousUpdateResponse.model_validate(i) for i in results]
            else:
                raise ValueError(f'input type {type(results[0])} invalid')
        else:
            raise ValueError(f'input type {type(results)} invalid')
    
        return inputs

# %% ../nbs/01_schemas.ipynb 24
UpdateFunction = Callable[Batch, UpdateResponse]
